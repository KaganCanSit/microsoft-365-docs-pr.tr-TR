...
description: Uç Nokta için Microsoft Defender'da Makine Eylemleri koleksiyonu almak için List MachineActions API'sini nasıl kullanabileceğinizi öğrenin.
keywords: api'ler, grafik api'leri, desteklenen api'ler, makineaction koleksiyonu
...
---
# <a name="list-machineactions-api"></a>List MachineActions API'si
...
Makine Eylemleri koleksiyonunu [sağlar](machineaction.md).

[OData V4 sorgularını destekler](https://www.odata.org/documentation/).

OData'nın `$filter` sorgusu şu şekilde destekler: `id`, `status`, `machineId`, `type`ve `requestor``creationDateTimeUtc` özellikleri.

`$stop` maksimum değeri 10.000
...
Başarılı olursa, bu yöntem makineAction varlıkları koleksiyonuyla birlikte 200 [, Tamam yanıt kodu](machineaction.md) döndürür.

## <a name="example-1"></a>Örnek 1

### <a name="example-1-request"></a>Örnek 1 istek

Üç MachineActions'a sahip bir kuruluşa yapılan istek için bir örnek şudur.
...
Burada, MachineActions'a makine kimliğine göre filtre uygulama ve en son iki MachineActions'i gösteren bir istek örneği bulunmaktadır.
