...
- **Şu** anda kullanıma sunulan yeniden etiketleme, etiketin saklama süresinin sonunda başka bir etiket uygulayacak şekilde yapılandırılıp yapılandırılmamış olduğunu tanımlar. Geçerli değerler:
...
1. **Dosya planı** sayfasında **+ Etiket** >  oluştur **Bekletme etiketi'ni** seçin
...
3. Etiketi oluşturduktan ve etiketi yayımlama seçeneklerini gördükten sonra, etiketi otomatik olarak uygulayın veya yalnızca etiketi kaydedin: **Şimdilik etiketi kaydet'i** ve ardından **Bitti'yi** seçin.
...
Şu anda [yayımlanmış](create-apply-retention-labels.md) veya [otomatik uygulama](apply-retention-labels-automatically.md) bekletme etiketi ilkelerine dahil olmayan, olay tabanlı saklama için yapılandırılmamış veya öğeleri düzenleyici kayıt olarak işaretleyen bekletme etiketlerini silebilirsiniz.

Silebileceğiniz bekletme etiketleri için, öğelere uygulandıysa silme işlemi başarısız olur ve etiketlenmiş öğeleri tanımlamak için içerik gezgininin bağlantısını görürsünüz.
...
4. Doldurulmuş şablonu karşıya yüklemek için **Karşıya dosya yükle'yi** seçin.
...
|CitationJurisdiction|Dize|Hayır|Bu özellik **, Sağlama/alıntı** dosya planı tanımlayıcısında görüntülenen yargı yetkisini veya kuruluşu belirtir. Örneğin, "ABD Menkul Kıymetler ve Borsa Komisyonu (SEC)".|
...
|EventType|Dize|**Hayır, RetentionType** **EventAgeInDays** değilse|Bu özellik, olay [tabanlı saklama](event-driven-retention.md) için kullanılan bir olay türünü belirtir. **Kayıt yönetimi** > **Olayları** >  Olay türlerini yönetme bölümünde görüntülenen mevcut bir **olay türünü** belirtin. Alternatif olarak, kullanılabilir olay türlerini görüntülemek için [Get-ComplianceRetentionEventType](/powershell/module/exchange/get-complianceretentioneventtype) cmdlet'ini kullanın. **Çalışan etkinliği** ve **Ürün ömrü** gibi bazı yerleşik olay türleri olsa da, kendi olay türlerinizi de oluşturabilirsiniz. </br> </br> Kendi olay türünüzü belirtirseniz, ad içeri aktarma işleminin bir parçası olarak doğrulandığından içeri aktarma işleminden önce var olması gerekir.|
|IsRecordUnlockedAsDefault|Dize|Hayır|Bu özellik, etiket uygulandığında kayıt olarak işaretlenen öğenin [kilidinin açılıp](record-versioning.md) açılmayacağını belirtir. Geçerli değerler şunlardır: </br>**DOĞRU**: Etiket uygulandığında kayıt olarak işaretlenmiş öğenin kilidi açılır. **IsRecordLabel** özelliği TRUE olarak ayarlanmalıdır ve **Mevzuat** özelliği TRUE olarak ayarlanamaz.</br>**YANLIŞ**: Öğe kayıt olarak işaretlenmez veya kayıt olarak işaretlenir ancak etiket uygulandığında kilitlenir. Bu, varsayılan değerdir.|
|ComplianceTagForNextStage|Dize|Hayır|Bu özellik, saklama süresinin sonunda uygulanacak [bir değiştirme etiketinin](retention-settings.md#relabeling-at-the-end-of-the-retention-period) adını belirtir. </br> </br> **Mevzuat** **TRUE** ise bu özelliği belirtmeyin.|
