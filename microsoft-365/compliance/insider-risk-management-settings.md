...
- **Risk puanı ilkeler**: Bunlar, olağandışı etkinlikler veya geçmiş ilke ihlalleri için risk puanını yükseltmeyi içerir. Risk puanlarının etkinleştirilmesi risk puanlarını ve bu tür etkinlikler için uyarı olasılığını artırır. Olağan dışı etkinlikler için, algılanan etkinlik kullanıcının normal davranışından sapsa puanlar artırıldı. Örneğin, günlük dosya indirmelerde önemli bir artış olur. Alışılmış dışı etkinlik yüzde oranı olarak (örneğin, 'normal etkinliğin üzerinde %100' olarak) ve risk puanı, etkinliklere bağlı olarak farklı etkiler. Önceki ilke ihlallerine sahip kullanıcılar için, kullanıcının daha önce onaylandı olarak onaylanmış ilke ihlalini çözmüş birden çok durumu varsa puanlar artırıldı. Risk puanı, ancak bir veya daha fazla gösterge seçiliyse seçilebilir.
...
3. Bulut Chromium etkinliğinin eylemlerini izlemek için uç nokta cihazına Microsoft Chromium Edge tarayıcısını yükleyin. Bkz[. Temel Microsoft Edge yeni Chromium](https://support.microsoft.com/help/4501095/download-the-new-microsoft-edge-based-on-chromium).
...
Uç Nokta için [Microsoft Defender'a önceden cihazlarınız](/windows/security/threat-protection/) varsa, bunlar yönetilen cihazlar listesinde zaten görünür. [3. Adımı izleyin: Sonraki bölümde Uç nokta için Microsoft Defender'a cihaz](insider-risk-management-settings.md#OnboardStep3) girdiyebilirsiniz.
...
MacOS cihazları (Catalina 10.15 veya sonrası) Intune veya JAMF Microsoft 365 kullanarak insider risk yönetimi ilkelerini desteklemek için Pro. Daha fazla bilgi ve yapılandırma kılavuzu için bkz. [macOS cihazlarını genel Microsoft 365 (önizleme) kullanma](device-onboarding-macos-overview.md).
...
Akıllı algılama ayarları, uyarılarda riskli etkinliklerin algılanmasında nasıl işlendiğine yardımcı olur. Bazı durumlarda, yoksaymak istediğiniz dosya türlerini tanımlamanız veya uyarılar için minimum çıtayı tanımlamanıza yardımcı olması için dosyalar için bir algılama düzeyi zorunlu tutulmaktadır. Genel uyarı sesi, dosya türü dışlamaları ve dosya ses düzeyi sınırlarını kontrol etmek için bu ayarları kullanın.
...
### <a name="threshold-for-unusual-file-activity"></a>Sıra dışı dosya etkinliği eşiği

Etkinlik uyarıları Insider risk ilkelarında bildirmeden önce en düşük dosya düzeyini tanımlamak için dosya sayısını girin. Örneğin, bir kullanıcı 10 veya daha az dosya indirirken, ilkeler bu etkinliği alışılmış dışı olarak düşünse bile insider risk uyarıları oluşturmak istemiyorsanız '10' girebilirsiniz.
...
İlgilenilen sinyal türlerine bağlı olarak, Uç nokta için Defender uyarı önceliği durumuna bağlı olarak uyarıları Insider risk yönetimine aktarmayı seçebilirsiniz. İçeri aktarma işlemi için genel ayarlarda aşağıdaki uyarı üç durumlarından birini veya birden fazlasını tanımlayabilirsiniz:
...
Örneğin, kullanıcıların hassas bilgilere erişimi olduğu çok gizli bir projede veri sızıntılarına karşı korumanız gerekir. Bu proje üzerinde *çalışan kullanıcılar Project* *için* Gizli kullanıcı grubu oluşturma Project Öncelik kullanıcı grubu. Buna ek olarak, bu öncelik kullanıcı grubunun, grupla ilişkilendirilmiş kullanıcıları, uyarıları, vakaları ve raporları tüm varsayılan insider risk yönetimi yöneticileri, analistleri ve tahminleri tarafından görülemeyebilir. Daha **Ayarlar** için Gizli kullanıcı *Project* Kullanıcılar öncelik kullanıcıları grubunu oluşturabilir ve iki kullanıcıya, gruplarla ilgili verileri görüntüley gerçekten gözden geçirici olarak atayabilirsiniz. İlke sihirbazını ve Öncelik  kullanıcıları tarafından veri sızıntıları ilke şablonunu kullanarak, yeni bir ilke oluşturur ve Gizli *Project* Kullanıcılar öncelik kullanıcıları grubunu ilkeye atarsınız. Gizli *Project* Kullanıcıları önceliğe sahip kullanıcı grubu üyeleri için ilke tarafından inceleyen etkinlikler risklere ve bu kullanıcıların etkinliklerine daha duyarlı olur ve bu kullanıcıların bir uyarı oluşturma olasılığı daha yüksektir ve daha yüksek önem düzeyine sahip uyarılar içerirler.
...
- **Bir kullanıcının insider risk uyarısı** olduğunda yöneticiye durumu bildir: Bazı kuruluşların, kullanıcıda insider risk yönetimi uyarısı olduğunda hemen yönetim uyarısı verilmesi gerekiyor olabilir. Bu akış yapılandırıldığında ve seçildiğinde, olay yöneticisine tüm olay uyarıları hakkında aşağıdaki bilgileri içeren bir e-posta iletisi gönderilir:
    ...
    Akış, iletinin gönderildiği ve akışın etkinleştirildiğinde olay notlarını otomatik olarak ler. Gizlilik ayarlarında kullanıcıları anonimleştirmeyi seçtiysiniz **, bu** şablondan oluşturulan akışlar, kullanıcı gizliliğinin korunması için hedeflenen şekilde çalışmaz. Power Automate şablon kullanan diğer akışlara Cases **panosundan da sknz.**
...
5. Gerekirse, Yeni adım düğmesini seçerek akışa başka **adımlar ekleyin** . Çoğu durumda, önerilen varsayılan şablonlar için bu gerekli değildir.
