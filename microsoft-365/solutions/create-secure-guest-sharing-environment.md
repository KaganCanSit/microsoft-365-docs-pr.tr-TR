...
Bu örnekte, Azure Active Directory koşullu erişim ilkesi kullanarak konuklar için çok faktörlü kimlik doğrulamasını ayarlayacağız.
...
Kullanım koşulları oluşturmak için, önce belgeyi Word'de veya başka bir yazma programında oluşturmanız ve ardından .pdf dosyası olarak kaydetmeniz gerekir. Bu dosya daha sonra Azure AD'ye yüklenebilir.
...
8. **Seç** dikey penceresinde **Microsoft Teams**, **Çevrimiçi Office 365 SharePoint** ve **Gruplar'ı Outlook** ve ardından **Seç'e** tıklayın.
...
Azure AD'deki erişim gözden geçirmeleriyle, çeşitli ekiplere ve gruplara kullanıcı erişiminin düzenli olarak gözden geçirilmesini otomatikleştirebilirsiniz. Konuklar için özel olarak bir erişim gözden geçirmesi gerektirerek, konukların kuruluşunuzun hassas bilgilerine erişimi gerekenden daha uzun süre saklamamalarına yardımcı olabilirsiniz.
...
4. **Konuk kullanıcılarla tüm Microsoft 365 grupları** seçeneğini belirleyin. Herhangi bir **grubu dışlamak istiyorsanız dışlamak** için Grupları seç'e tıklayın.
...
10. **Bitiş** için **Hiçbir Zaman'ı** seçin ve ardından **İleri: Ayarlar'e** tıklayın.
...
SharePoint ve OneDrive konumlar için, belgenin tüm konuklar için paylaşılıp paylaşılmadığına bakılmaksızın, hassas bilgiler algıladıktan hemen sonra belgelerin proaktif olarak engellendiğini ve dahili kullanıcıların belgeye erişmeye devam edeceklerini unutmayın.
...
Saldırı yüzeyinizi azaltabilir ve konukların yalnızca bir web tarayıcısı kullanarak ekiplerinize, sitelerinize ve dosyalarınıza erişmesini gerektirerek yönetimi kolaylaştırabilirsiniz.

Microsoft 365 Grupları ve Teams için bu işlem bir Azure AD koşullu erişim ilkesiyle yapılır. SharePoint için bu, SharePoint yönetim merkezinde yapılandırılır. (Konukları [yalnızca web erişimine kısıtlamak için duyarlılık etiketlerini de kullanabilirsiniz](../compliance/sensitivity-labels-teams-groups-sites.md).)

Grupların ve Teams için konukları yalnızca web erişimiyle kısıtlamak için:
...
6. **Atamalar'ın** altında **Bulut uygulamaları veya eylemleri'ne** tıklayın.
7. **Ekle** sekmesinde **Uygulamaları seç'i** seçin ve **seç'e** tıklayın.
8. **Seç** dikey penceresinde **Microsoft Teams** ve **Outlook Grupları'nı** seçip **Seç'e** tıklayın.
...
14. **Ver** dikey penceresinde **Cihazın uyumlu olarak işaretlenmesini gerektir** ve **Hibrit Azure AD'ye katılmış cihaz gerektir'i** seçin.
...
Konukları SharePoint için web üzerinden erişimle kısıtlamak için
...
SharePoint yönetim merkezindeki bu ayarın Azure AD'de destekleyici bir koşullu erişim ilkesi oluşturduğunu unutmayın.

## <a name="configure-a-session-timeout-for-guests"></a>Konuklar için oturum zaman aşımı yapılandırma

Konukların düzenli olarak kimlik doğrulaması yapmalarını zorunlu tutmak, bir konuğun cihazı güvenli tutulmazsa bilinmeyen kullanıcıların kuruluşunuzun içeriğine erişme olasılığını azaltabilir. Azure AD'de konuklar için oturum zaman aşımı koşullu erişim ilkesi yapılandırabilirsiniz.
...
1. [Microsoft Purview uyumluluk portalının](https://compliance.microsoft.com) sol gezinti bölmesinde **Sınıflandırma'yı** genişletin ve **hassas bilgi türleri'ne** tıklayın.
2. **Oluştur'a** tıklayın.
3. **Ad** ve **Açıklama** için **Project Satürn** yazın ve **İleri'ye** tıklayın.
4. **Öğe ekle'ye** tıklayın.
5. İçerik **içeren içeriği algıla** listesinde **Anahtar Sözcükler'i** seçin ve anahtar sözcük kutusuna *Project Satürn* yazın.
...
6. **Etiket uygulamak istediğiniz konumları seçin** sayfasında, **siteleri SharePoint** açın ve **Siteleri seç'e** tıklayın.
...
İlke uygulandığında, kullanıcı belgeye "Project Satürn" yazdığınızda, otomatik etiketleme ilkesi dosyayı taradığında belirtilen etiketi otomatik olarak uygular.
...
11. **Erişimi kısıtla veya Microsoft 365 konumlardaki içeriği şifrele** onay kutusunu ve ardından **Yalnızca kuruluşunuzun dışındaki kişiler** seçeneğini belirleyin.
...
Microsoft 365 ve Azure Active Directory konuk paylaşım ortamınızın güvenliğini sağlamaya yardımcı olabilecek bazı ek seçenekler vardır.
...
- Kullanıcılarınızın bağlanabileceği diğer Azure Active Directory kiracıları sınırlayabilirsiniz. Bilgi için bkz. [SaaS bulut uygulamalarına erişimi yönetmek için kiracı kısıtlamalarını kullanma](/azure/active-directory/manage-apps/tenant-restrictions) .
